  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const fetchedProfiles = await fetchTestProfiles();
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="max-w-3xl text-left mb-12 lg:mb-16 space-y-4">
          ...
          <h1 className="font-serif text-4xl sm:text-5xl font-light tracking-tight text-brand-charcoal leading-none">
            Clinical Diagnostic <span className="bg-gradient-to-r from-brand-emerald to-brand-emerald-dark bg-clip-text text-transparent font-normal">Catalog</span> <br />
        ...
        {/* Results List Grid */}
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 bg-brand-white border border-brand-charcoal/5 rounded-3xl shadow-xs">
            <Loader2 className="h-10 w-10 animate-spin text-brand-emerald" />
            <p className="mt-4 text-[10px] font-bold uppercase tracking-wider text-brand-charcoal/40">Synchronizing Database...</p>
          </div>
